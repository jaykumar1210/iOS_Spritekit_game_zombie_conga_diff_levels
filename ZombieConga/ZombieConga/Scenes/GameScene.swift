...
class GameScene: SKScene {
  ...
  override func didMove(to view: SKView) {
    ...
    mutebutton.position = CGPoint(x: playableRect.size.width/2 - CGFloat(50),
  ...
  func spawnCat() {
    ...
    /***** Logic 3: Including the game logic above, additionally, old lady could show up from both left and right *//////
    /*Start*/
    
    
    func spawnEnemyFromLeft() {
        let enemyFromLeft = SKSpriteNode(imageNamed: "enemy")
        
        enemyFromLeft.position = CGPoint(
            x: cameraRect.maxX - enemyFromLeft.size.width * 10,
            y: CGFloat.random(
                min: cameraRect.minY + enemyFromLeft.size.height/2,
                max: cameraRect.maxY - enemyFromLeft.size.height/2))
        enemyFromLeft.zPosition = 150
        enemyFromLeft.name = "enemy"
        enemyFromLeft.setScale(-1)
        addChild(enemyFromLeft)
        let actionMoveFromLeft =
            SKAction.moveBy(x: (size.width + enemyFromLeft.size.width)   , y: 0, duration: 3.0)
        let actionRemove = SKAction.removeFromParent()
        enemyFromLeft.run(SKAction.sequence([actionMoveFromLeft, actionRemove]))
    ...
    //***Logic 4: Including the game logic above, additionally, show small fish every 8 second. if the zombie get a small fish，in 4 second, the cat will not dismiss from the zombie even the old lady catch the zombie, the cat and the zombie change to colour orange。***////
  ...
  func zombieHit(enemy: SKSpriteNode) {
    ...
    func addCats(){
        ...
        enumerateChildNodes(withName: "cat") { node, _ in
            ...
            if Int(cat.position.x)-Int(self.zombie.position.x) <= 800 && Int(cat.position.y)-Int(self.zombie.position.y) <= 800{
                hitCats.append(cat)
             }
         }
         for cat in hitCats {
